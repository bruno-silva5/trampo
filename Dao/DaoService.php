    ...
    class DaoService {
        public function cadastrarService(Service $service) {
            ...
            if(!empty($service->getPicture())) {
                $query = "INSERT INTO service(time_remaining, title, description, picture, is_visible, id_occupation_subcategory, id_user) 
                VALUES ('".$service->getTimeRemaining()."','".$service->getTitle()."',
                '".$service->getDescription()."','".$service->getPicture()."','".$service->getIsVisible()."', '".$service->getIdOccupationSubcategory()."', '".$service->getIdUser()."')";      
            } else {
